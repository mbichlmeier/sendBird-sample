        ...
        doOnChannelUrl() {channelUrl->
            ...
            try {
                fragmentManager?.beginTransaction()?.replace(
                    ...
                    ChannelFragment.Builder("ThisIsNotAValidChannelUrl") // passing a valid [channelUrl] works as expected
                        .setUseTypingIndicator(true)
                        .setInputHint("Enter a message here")
                        .build()
                )?.commit()
            } catch (e: SendBirdException){
                Timber.e(e, "Could not open ChannelFragment. Even catching the exception does not work!")
            }
        }
    }
